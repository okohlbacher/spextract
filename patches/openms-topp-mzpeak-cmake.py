# Append the mzPeak link block for spextract to the OpenMS tree's TOPP CMakeLists (cluster).
...
if "SPEXTRACT_WITH_MZPEAK" in s: print("already applied"); raise SystemExit
s += '''
# [SpeXtract mzpeak] streaming .mzpeak input through the mzPeak C++ library (fork trunk).
if(TARGET spextract AND EXISTS /path/to/scratch/mzpeak-cpp/build/libmzpeak.a)
  set(_spx_env /path/to/shared/AI/OpenDIAlyzer/opt/env)
  target_compile_definitions(spextract PRIVATE SPEXTRACT_WITH_MZPEAK)
  target_compile_features(spextract PRIVATE cxx_std_23)
  target_include_directories(spextract PRIVATE /path/to/scratch/mzpeak-cpp/include /path/to/scratch/mzpeak-cpp/boost189inc ${CMAKE_SOURCE_DIR}/src/openms/extern/SQLiteCpp/include ${CMAKE_SOURCE_DIR}/src/openms/extern/SQLiteCpp/sqlite3)
  file(GLOB _spx_numpress /path/to/scratch/mzpeak-cpp/build/subprojects/msnumpress/*.a)
  target_link_libraries(spextract
  ...
  set_target_properties(spextract PROPERTIES BUILD_RPATH "${_spx_env}/lib;/path/to/scratch/contrib/build/lib")
endif()
'''
