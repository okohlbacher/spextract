...
REMOTE_SRC=/path/to/scratch/OpenMS/src/topp/spextract.cpp
...
scp -q src/TdfMzCalibration.h "$NODE":/path/to/scratch/OpenMS/src/openms/include/OpenMS/FORMAT/
...
ssh "$NODE" "grep -q TdfMzCalibration /path/to/scratch/OpenMS/src/openms/source/FORMAT/BrukerTimsFile.cpp" || {
  echo "REFUSED: the cluster OpenMS tree has no calibration patch applied." >&2
  echo "  run: scripts/apply_openms_patches.sh /path/to/scratch/OpenMS   (on the node)" >&2
  exit 1; }

ssh "$NODE" "set -eu
  mkdir /path/to/scratch/.deploy.lock 2>/dev/null || { echo 'REFUSED: another deploy holds the lock' >&2; exit 1; }
  trap 'rmdir /path/to/scratch/.deploy.lock' EXIT
  cp -p '$REMOTE_SRC' '${REMOTE_SRC}.bak' 2>/dev/null || true
  mv '${REMOTE_SRC}.new' '$REMOTE_SRC'
  cd /path/to/scratch/OpenMS/build
  if ! make spextract -j32 > /path/to/scratch/build_sync_\$(date +%Y%m%d-%H%M%S).log 2>&1; then
  ...
  EPD=/path/to/scratch/OpenMS/src/openms/source/FEATUREFINDER/ElutionPeakDetection.cpp
  ...
  cat bin/spextract.provenance | tee -a /path/to/scratch/deploys.log"
